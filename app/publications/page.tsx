  ...
  return (
    <div className="min-h-screen bg-[#fcfcfc] text-slate-900 font-sans selection:bg-blue-100">
      <main className="max-w-3xl mx-auto px-6 py-20 animate-fade-in">
        ...
        <h1 className="text-4xl font-black mb-16 italic tracking-tight uppercase">Research & Publications</h1>
        
        {/* Section 1: Published Works */}
        <section className="mb-20">
          <h2 className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em] mb-8">Academic & Media Publications</h2>
          <div className="space-y-12">
            {published.map((pub, i) => (
              <article key={i} className="group border-l border-slate-200 pl-6 ml-1">
                ...
                <p className="text-xs text-slate-500 mb-3 uppercase tracking-tighter italic">{pub.source}</p>
                <p className="text-slate-600 text-sm leading-relaxed">{pub.description}</p>
              </article>
            ))}
          </div>
        </section>
        ...
        <section>
          <h2 className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em] mb-8">Independent Research Papers</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {independent.map((res, i) => (
              <a 
                ...
                className="p-6 rounded-3xl bg-white border border-slate-100 shadow-sm hover:shadow-md hover:border-blue-200 transition-all group"
              >
                <div className="flex justify-between items-start mb-4">
                  <span className="text-[9px] font-bold px-2 py-1 bg-slate-100 rounded text-slate-500 uppercase tracking-widest">{res.type}</span>
                  <span className="text-blue-500 opacity-0 group-hover:opacity-100 transition-opacity">↗</span>
                </div>
                <h3 className="font-bold text-sm leading-tight mb-2 group-hover:text-blue-600">{res.title}</h3>
                <p className="text-[11px] text-slate-500 leading-normal line-clamp-3">{res.description}</p>
              </a>
            ))}
          </div>
        </section>

        <footer className="mt-40 text-center text-[10px] font-mono text-slate-300 uppercase tracking-[0.5em]">
